...
class RDPTest extends AnyFunSuite:
  test("Tokenizer") {
    ...
    assert(tokenize("(ab)?") === List(Tok_LPAREN,Tok_Char('a'),Tok_OR,Tok_Char('b'),Tok_RPAREN,Tok_Q, Token.Tok_End))
    assert(tokenize("ab|cd?") === List(Tok_Char('a'),Tok_Char('b'),Tok_OR,Tok_Char('c'),Tok_Char('c'),Tok_Q,Token.Tok_End))
  ...
  test("match") {
    ...
    assert(Exp.matcher (RDP.parse("I (like|love|hate)( (cat|dog))? people"), "I love dog people") === false)
